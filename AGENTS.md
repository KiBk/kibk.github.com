# Project summary
- Static personal portfolio for Kirill Bykov forked from Ryan Fitzgerald's devportfolio template; still a single-page site (`index.html`), but now with a responsive split hero, an always-open client-side portfolio terminal, richer project cards with expandable details, sections for About, Experience, Education, Projects, Skills, Languages/Personal Achievements, and a Formspree-backed contact form, plus a root `.nojekyll` file so GitHub Pages serves the site as raw static content.
